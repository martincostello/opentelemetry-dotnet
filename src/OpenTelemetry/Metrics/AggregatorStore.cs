...
internal sealed class AggregatorStore
{
    ...
    // map to themselves) to the array slot that holds it. MetricPoint is larger
    // than half a cache line, so points in adjacent slots always share a cache
    // line and updates to them from different threads contend with each other
    // (false sharing). Placing consecutively created points two slots apart (all
    // even offsets first, then all odd offsets) keeps the points a store creates
