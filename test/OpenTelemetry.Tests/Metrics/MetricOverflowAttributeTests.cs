...
public class MetricOverflowAttributeTests
{
    ...
    {
        // MetricPoints are assigned to array slots in a strided (even slots first,
        // then odd slots) order to avoid false sharing between points created
        // together. The mapping must remain a bijection for odd and even limits so
        // that the store can hold exactly the configured number of tag sets, and
        // cumulative streams must still export points in creation order.
        var exportedItems = new List<Metric>();
